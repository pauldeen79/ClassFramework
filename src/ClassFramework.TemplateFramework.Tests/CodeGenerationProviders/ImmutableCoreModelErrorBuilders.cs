...
public class ImmutableCoreModelErrorBuilders(IPipelineService pipelineService) : ImmutableCSharpClassBase(pipelineService)
{
    ...
    public override Task<Result<IEnumerable<TypeBase>>> GetModelAsync(CancellationToken cancellationToken) => GetBuildersAsync(Task.FromResult(Result.Error<IEnumerable<TypeBase>>("Kaboom")), "Test.Domain.Builders", "Test.Domain");
