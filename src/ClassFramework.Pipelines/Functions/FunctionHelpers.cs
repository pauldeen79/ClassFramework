...
internal static class FunctionHelpers
{
    ...
    internal static async Task<Result<string>> ParseFromContextAsync(FunctionCallContext context, Func<CommandBase, PipelineSettings, ClassModel, Property, bool, string> resultDelegate)
        => (await new AsyncResultDictionaryBuilder()
            ...
            .OnSuccess(results =>
            {
                var contextBase = results.GetValue<CommandBase>(ResultNames.Context);
    ...
    internal static async Task<Result<T>> ParseFromContextAsync<T>(FunctionCallContext context, string functionName, Func<CommandBase, Result<T>> functionDelegate)
    {
        var ctx = await context.Context.State[ResultNames.Context]().ConfigureAwait(false);

        return ctx.Value switch
        {
            CommandBase contextBase => functionDelegate(contextBase),
            ParentChildContext<GenerateEntityCommand, Property> parentChildContextEntity => functionDelegate(parentChildContextEntity.ParentContext),
