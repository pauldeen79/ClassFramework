...
public class AddCopyConstructorFeature : IPipelineFeature<IConcreteTypeBuilder, BuilderContext>
{
    ...
    private Result<ConstructorBuilder> CreateCopyConstructor(PipelineContext<IConcreteTypeBuilder, BuilderContext> context)
    {
        ...
        var constructorInitializerResults = GetConstructorInitializerResults(context, MetadataNames.CustomBuilderConstructorInitializeExpression);
    ...
    private Tuple<string, Result<string>>[] GetConstructorInitializerResults(PipelineContext<IConcreteTypeBuilder, BuilderContext> context, string metadataName)
        => context.Context.SourceModel.Properties
            ...
            (
                x.GetBuilderMemberName(context.Context.Settings.AddNullChecks, context.Context.Settings.EnableNullableReferenceTypes, context.Context.Settings.OriginalValidateArguments, context.Context.Settings.AddBackingFields || context.Context.Settings.CreateAsObservable, context.Context.FormatProvider.ToCultureInfo()),
