...
public class PropertyEntityMemberNameProperty : IProperty
{
    public async Task<Result<object?>> EvaluateAsync(FunctionCallContext context, CancellationToken token)
    {
        ...
        return await context.EvaluateForProperty(
            (property, settings) => property.GetEntityMemberName(settings.AddBackingFields || settings.CreateAsObservable || settings.FluentBuilderMethods, context.Context.Settings.FormatProvider.ToCultureInfo())).ConfigureAwait(false);
    }
}
