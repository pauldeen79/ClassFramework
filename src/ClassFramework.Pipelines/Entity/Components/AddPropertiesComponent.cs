...
public class AddPropertiesComponent : IPipelineComponent<GenerateEntityCommand, ClassBuilder>
{
    public Task<Result> ExecuteAsync(GenerateEntityCommand command, ClassBuilder response, ICommandService commandService, CancellationToken token)
        => Task.Run(() =>
        {
            ...
            response.AddProperties(
                ...
                (
                    property => command.CreatePropertyForEntity(property)
                        ...
                        .WithHasInitializer(property.HasInitializer && !(command.Settings.AddSetters || command.Settings.AddBackingFields || command.Settings.CreateAsObservable || command.Settings.FluentBuilderMethods))
                        .WithHasSetter(((command.Settings.AddSetters || command.Settings.AddBackingFields) && !property.TypeName.IsCollectionTypeName()) || command.Settings.CreateAsObservable || command.Settings.FluentBuilderMethods)
            ...
            if (command.Settings.AddBackingFields || command.Settings.CreateAsObservable || command.Settings.FluentBuilderMethods)
            {
